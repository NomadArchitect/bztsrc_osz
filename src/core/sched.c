...
#include <platform.h>               /* szükségünk van az INTR_SCHEDREMAINDER definícióra */
...
void sched_pick()
{
    tcb_t *tcb = (tcb_t*)LDYN_tmpmap1, *tcba = (tcb_t*)LDYN_tcbalarm;
    ...
    if(ccb->hd_active[i] == 0) {
        ...
        if(runlevel == RUNLVL_COOP) {
            drivers_ready();
            /* a BSP-n az FS taszkot kell beütemezni, ha végeztünk a meghajtók inicializálásával */
            if(!ccb->id) { i = PRI_SRV; goto found; }
        }
...
found:
    ...
    if(ccb->current != tcb->pid) {
        ...
        if(runlevel == RUNLVL_NORM) {
            ...
            /* ha a mostani taszk megszakítható, és nem az idle taszk, akkor legkésőbb quantum mszek múlva meg is kell szakítani */
            ...
            if(ccb->sched_delta) {
                ...
                intr_enable(sched_irq);
            } else
                intr_disable(sched_irq);
        }
    }
}
