        ...
        if(!serial) {
            ...
            if(MSG_ISRESP(msg->evt)) msg = NULL;
                ...
                if(MSG_ISPTR(msg->evt)) msghdr->mq_bufstart =
                    ((((virt_t)msg->data.buf.ptr + msg->data.buf.size + __PAGESIZE-1) >> __PAGEBITS) << __PAGEBITS);
            }
        } else {
            ...
            if(MSG_ISRESP(msg->evt) || msg->serial != serial) msg = NULL;
                ...
                if(i == j) {
                    msghdr->mq_end = j;
                    if(MSG_ISPTR(msg->evt)) msghdr->mq_bufend = (virt_t)msg->data.buf.ptr & ~(__PAGESIZE-1UL);
                } else {
